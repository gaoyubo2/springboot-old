...
<%
    ...
    List<String> byUsername = (List<String>) request.getAttribute("byUsername");
    List<AppCategoryModel> appEditList = (List<AppCategoryModel>) request.getAttribute("appEditList");
%>
<!doctype html>
<html>
    <head>
    ...
    <body>
        <form action="${pageContext.request.contextPath}/appMarketController/save.do" method="post" name="form"
              id="form">
            <input type="hidden" name="id" value="<%=app.getTbid()%>">
            <div class="creatbox">
                <div class="middle">
                    <%
                        if (app.getVerifytype() == 0 || app.getVerifytype() == 3) {
                    %>
                    <p class="detile-title">编辑应用</p>
                    <%
                    } else {
                    %>
                    <p class="detile-title">查看应用</p>
                    <%}%>
                    ...
                    <%
                        if (app.getVerifytype() == 0) {
                    %>
                    <div class="input-label">
                        <label class="label-text">是否提交审核：</label>
                        <div class="label-box form-inline control-group">
                            <label class="radio" style="margin-right:10px"><input type="radio" name="val_verifytype"
                                                                                  value="0" checked>暂时不提交</label>
                            <label class="radio"><input type="radio" name="val_verifytype" value="2">提交审核</label>
                        </div>
                    </div>
                    <%
                    } else if (app.getVerifytype() == 3) {
                    %>
                    <div class="input-label">
                        <label class="label-text">是否提交审核：</label>
                        <div class="label-box form-inline control-group">
                            <label class="radio" style="margin-right:10px"><input type="radio" name="val_verifytype"
                                                                                  value="0">暂时不提交</label>
                            <label class="radio"><input type="radio" name="val_verifytype" value="2"
                                                        checked>重新提交审核</label>
                        </div>
                        <div class="label-box form-inline control-group">
                            <span class="help-inline">应用审核失败，失败原因：<%=app.getVerifyinfo()%></span>
                        </div>
                    </div>
                    <%}%>
                </div>
            ...
            <div class="bottom-bar">
                <div class="con">
                    ...
                    <a class="btn btn-primary fr" id="btn-submit" href="javascript:;"><i class="icon-white icon-ok"></i>
                        确定</a>
                    <%}%>
                    <a class="btn fr" href="javascript:window.parent.closeDetailIframe();" style="margin-right:10px"><i
                            class="icon-chevron-up"></i> 返回应用列表</a>
                </div>
            </div>
        </form>
        ...
        <script>
            ...
            $('#val_name').on("blur", function (event) {
                ...
                arrays[<%=i%>] = " <%=byUsername.get(i)%> ";

                if (value === "<%=byUsername.get(i)%>") {
                    if(isequal == false){
                        $.dialog({
                            content: '应用名已存在! 请更换!'
                        });
